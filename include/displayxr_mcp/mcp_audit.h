 ...
 * @ingroup aux_util
 *
 * Every call to mcp_audit_append() writes one line:
 *
 ...
 * are logged via MCP_LOG_W and otherwise ignored.
 */
void
mcp_audit_append(const char *tool, uint32_t client_id, uint64_t args_hash);
 ...
 * mcp_audit_append initializes lazily).
 */
const char *
mcp_audit_path(void);
